...
![Integrated Architecture](./01-api-data-pipeline/data%20pipeline.png)

### 💡 Core Business Impact

* **회계 기준 매출 YoY +59% 성장** 견인 기반 마련
* **Daily Accrual 엔진 구축**: 재무 장부와 **97.2% 일치**하는 실질 매출 정합성 확보
* **MTA 모델 고도화**: 7-Day Lookback 및 기여도 분석으로 성과 측정 사각지대 제거
* **업무 효율화**: 매일 진행되던 수동 정산 및 리포트 업무를 **100% 자동화**
...
1. [**01. API 기반 데이터 파이프라인**](./01-api-data-pipeline)
   - 파편화된 데이터 통합 및 Full-Funnel 데이터 마트 구축
2. [**02. S2S 전환 트래킹 시스템**](./02-s2s-conversion-tracking)
   - 쿠키리스 환경 대응을 위한 서버 간 직접 데이터 연동 (CAPI)
3. [**03. GFA 수집 자동화**](./03-gfa-crawling-automation)
   - API 미지원 환경을 극복한 Headless Browser 기반 자동화
...
6. [**06. 웹 퍼널 분석 및 데이터 기반 CRO 런칭 (Cross-functional)**](./06-web-funnel-conversion-analysis)
   - 원천 행동 로그 분석 기반 유저 주요 이탈 지점 진단 및 디자이너/퍼블리셔 협업을 통한 페르소나 소구형 신규 랜딩페이지 배포 완료
